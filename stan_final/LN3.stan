functions{
   ...
   real cen_lognormal_lpdf(real t, real is_censored, real muU, real sigma, row_vector z, vector betaC, real lambda_rp) {
    real log_lik_cen;
    real log_lik_uncen;
    real prop; 
    real prob = 0;
      
      prop = 1-pow(1-inv_logit(z * betaC), lambda_rp); // cured probability 
...
model{
  ...
  for (i in 1:N){
    // real log_lik_i = lik_bern_lpmf(cured[i]| z[i], betaC, lambda_rp);
    real log_lik_t = cen_lognormal_lpdf(t[i]|is_censored[i], muU[i], sigma, z[i], betaC, lambda_rp);
    target += log_lik_t;
  }
}
...
generated quantities{
  vector[N] log_lik; 
  for (i in 1:N) {
    log_lik[i] = cen_lognormal_lpdf(t[i]|is_censored[i], muU[i], sigma, z[i], betaC, lambda_rp);
  }
}
