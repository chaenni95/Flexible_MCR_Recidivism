...
w <- scale(matrix(runif(n*(q),-1,1),n,q)) # no intercept for rplogit!
...
for (i in 1:num_reps) {
  ...
  data <- geraMCM.Lognormal(n, x, w, censoring_time=censoring_time , beta=beta, eta=eta, alpha=alpha,link="rplogit", tau=1.5, delta = 1)  # Custom function to simulate data
  ...
  fit <- sampling(stanmodel, data = stan_data, seed = 12345, chains = 1, iter = 2000, 
                  init = 0, pars = c("betaU", "betaC", "sigma", "lambda_rp", "lp__", "log_lik"))
...
# saveRDS(results, "results/result1000/simln3.rds")



# Step 2: Apply the function to all simulation results
simulation_summaries <- lapply(results, extract_results)
saveRDS(simulation_summaries, "results/result1000/simulation_summaries.ln3.1000.rds")
...
dist <- "lognormal"
link <- "rplogit"
tag <- paste(dist, link, sep = "_")  # e.g., "lognormal_rplogit"
