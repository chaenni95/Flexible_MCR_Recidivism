...
for (i in 1:num_reps) {
  ...
  data <- geraMCM.Lognormal(n, x, w, censoring_time=censoring_time , beta=beta, eta=eta, alpha=alpha,link="logit", tau=1, delta = 1)  # Custom function to simulate data
  ...
  fit <- sampling(stanmodel, data = stan_data, seed = 12345, chains = 1, iter = 2000, 
                  init = 0, pars = c("betaU", "betaC", "sigma", "lp__", "log_lik"))
...
# saveRDS(results, "results/result1000/simln1.rds")



# Step 2: Apply the function to all simulation results
simulation_summaries <- lapply(results, extract_results)
saveRDS(simulation_summaries, "results/result1000/simulation_summaries.ln1.1000.rds")
...
cs_all <- lapply(results, extract_cs_residual, dist = "lognormal", link = "logit")
...
dist <- "lognormal"
link <- "logit"
tag <- paste(dist, link, sep = "_")  # e.g., "lognormal_logit"
