...
for (i in 1:num_reps) {
  ...
  data <- geraMCM.WB(n, x, w, censoring_time=censoring_time , beta=beta, eta=eta, alpha=alpha,link="logit", tau=1, delta = 1)  # Custom function to simulate data
...
# saveRDS(results, "results/result1000/simwb1.rds")



# Step 2: Apply the function to all simulation results
simulation_summaries <- lapply(results, extract_results)
saveRDS(simulation_summaries, "results/result1000/simulation_summaries.wb1.1000.rds")
...
cs_all <- lapply(results, extract_cs_residual, dist = "weibull", link = "logit")
...
#### save data for drawing cox-snell 
dist <- "weibull"
link <- "logit"
