...
for (i in 1:num_reps) {
  ...
  data <- geraMCM.Loglogistic(n, x, w, censoring_time=censoring_time , beta=beta, eta=eta, alpha=alpha,link="rplogit", tau=1.5, delta = 1)  # Custom function to simulate data
...
# saveRDS(results, "results/result1000/simllg3.rds")


# Step 2: Apply the function to all simulation results
simulation_summaries <- lapply(results, extract_results)
saveRDS(simulation_summaries, "results/result1000/simulation_summaries.llg3.1000.rds")
...
cs_all <- lapply(results, extract_cs_residual, dist = "loglogistic", link = "rplogit")
